          ...
          if (order.status === 'SHIPPED' || order.status === 'INVOICED' || order.status === 'PAID') {
            entries.push({
              ...
              description: order.invoiceNumber 
                ? this.translate.instant('generalLedger.description.revenueWithInvoice', { orderNumber: order.orderNumber || '', invoiceNumber: order.invoiceNumber })
          ...
          if (order.status === 'SHIPPED' || order.status === 'INVOICED' || order.status === 'PAID') {
            ...
            if (totalCost > 0) {
              entries.push({
                ...
                customerName,
                description: (order.invoiceNumber 
                  ? this.translate.instant('generalLedger.description.costWithInvoice', { orderNumber: order.orderNumber || '', invoiceNumber: order.invoiceNumber })
                  : this.translate.instant('generalLedger.description.cost', { orderNumber: order.orderNumber || '' })) 
                  + this.translate.instant('generalLedger.description.costDetails', { productCost: productCost.toFixed(2), shippingCost: shippingCost.toFixed(2) }),
                quantity: totalQuantity,
                amount: totalCost,
                cost: totalCost,
                status: order.status || '',
          ...
          if (order.status === 'PAID' && order.jsonData?.paymentAmount) {
            ...
            if (paymentAmount > 0) {
              entries.push({
                ...
                customerName,
                description: order.invoiceNumber 
                  ? this.translate.instant('generalLedger.description.paymentWithInvoice', { orderNumber: order.orderNumber || '', invoiceNumber: order.invoiceNumber })
        ...
        processedPOs.forEach(po => {
            ...
            if (totalCost > 0) {
              entries.push({
                ...
                description: (po.invoiceNumber 
                  ? this.translate.instant('generalLedger.description.expenseWithInvoice', { poNumber: po.orderNumber || '', invoiceNumber: po.invoiceNumber })
                  : this.translate.instant('generalLedger.description.expense', { poNumber: po.orderNumber || '' })) 
                  + this.translate.instant('generalLedger.description.expenseDetails', { productCost: productCost.toFixed(2), shippingCost: shippingCost.toFixed(2) }),
                quantity: totalQuantity,
                amount: totalCost,
                cost: totalCost,
                status: po.status || '',
          ...
          if (po.status === 'INVOICED' || po.status === 'PAID') {
            entries.push({
              ...
              supplierName,
              description: po.invoiceNumber 
                ? this.translate.instant('generalLedger.description.apWithInvoice', { poNumber: po.orderNumber || '', invoiceNumber: po.invoiceNumber })
          ...
          if (po.status === 'PAID' && po.jsonData?.paymentAmount) {
            ...
            if (paymentAmount > 0) {
              entries.push({
                ...
                supplierName,
                description: po.invoiceNumber 
                  ? this.translate.instant('generalLedger.description.poPaymentWithInvoice', { poNumber: po.orderNumber || '', invoiceNumber: po.invoiceNumber })
